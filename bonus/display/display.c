...
/*   Updated: 2024/07/08 17:56:29 by aurodrig         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	key_loop(t_long *sl)
{
	...
	{
		fixbomb(sl);
		mlx_string_put(sl->mlx_ptr, sl->mlx_win, 10, 20, 0xffffff, "MOVE:");
		mlx_string_put(sl->mlx_ptr, sl->mlx_win, 75, 20, 0xffffff, line);
		free(line);
	}
	sl->dropbomb++;
	ft_keyboard(sl);
...
int	createwindow(t_long *sl)
{
	...
	sl->mlx_win = mlx_new_window(sl->mlx_ptr, sl->xscreen,
			sl->yscreen, "42 so_long aurodrig");
	return (0);
}
